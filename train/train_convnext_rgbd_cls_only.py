"""
TransUNet ConvNeXt RGBD 仅分类训练脚本
用于多任务学习消融实验 - 仅分类任务 (带深度图)

用法:
    python train_convnext_rgbd_cls_only.py --config base
    python train_convnext_rgbd_cls_only.py --config debug

与双任务版本的区别:
- 仅使用分类 Loss 训练
- 模型没有分割头
- 只评估分类指标
- 使用 5 通道输入 (RGB + Depth + Mask)
"""
...
from train.models.transunet_convnext_rgbd_cls_only import (
    TransUNetConvNeXtRGBDClsOnly, 
    TransUNetConvNeXtRGBDClsOnlyConfig,
    transunet_convnext_rgbd_cls_only_base, 
    transunet_convnext_rgbd_cls_only_small, 
    transunet_convnext_rgbd_cls_only_tiny
)
...
def create_model_rgbd_cls_only(config: Config, device: torch.device) -> nn.Module:
    """创建 RGBD 仅分类模型"""
    model_config = config.model
    
    if model_config.variant == 'tiny':
        model = transunet_convnext_rgbd_cls_only_tiny(
        ...
        model = transunet_convnext_rgbd_cls_only_small(
            pretrained=model_config.pretrained,
            img_height=config.data.img_height,
            img_width=config.data.img_width,
            dropout=model_config.dropout,
        ...
        model = transunet_convnext_rgbd_cls_only_base(
            pretrained=model_config.pretrained,
            img_height=config.data.img_height,
            img_width=config.data.img_width,
            dropout=model_config.dropout,
    ...
    print(f"模型: TransUNet-ConvNeXt-RGBD-ClsOnly-{model_config.variant}")
    print(f"任务: 仅分类 (无分割头)")
...
def train_one_epoch(
    ...
    metrics['cls_loss'] = loss_meter.avg
...
def train(config: Config):
    ...
    model = create_model_rgbd_cls_only(config, device)
    ...
    if hasattr(config, 'resume_path') and config.resume_path:
        ...
        if 'metrics' in checkpoint and 'val' in checkpoint['metrics']:
            best_f1 = checkpoint['metrics']['val'].get('cls_f1', 0.0)
    
    print("\n" + "=" * 60)
    print("开始训练 (RGBD 仅分类任务)...")
    print("=" * 60)
    ...
    for epoch in range(start_epoch, config.train.epochs + 1):
        ...
        print(f"\n训练: loss={train_metrics['loss']:.4f}, acc={train_metrics['cls_accuracy']:.4f}, f1={train_metrics['cls_f1']:.4f}")
...
def get_config_rgbd_cls_only(preset: str = 'default') -> Config:
    """获取 RGBD 仅分类预设配置"""
    ...
    if preset == 'debug':
        return Config(
            ...
            exp_name='debug_convnext_rgbd_cls_only',
        )
    elif preset == 'base':
        return Config(
            ...
            exp_name='transunet_convnext_rgbd_cls_only_base',
        )
    else:  # default
        return Config(
            data=DataConfig(
            ...
            exp_name='transunet_convnext_rgbd_cls_only',
        )


def parse_args():
    parser = argparse.ArgumentParser(description='TransUNet ConvNeXt RGBD 仅分类训练')
...
def main():
    args = parse_args()
    config = get_config_rgbd_cls_only(args.config)
    ...
    print("\n" + "=" * 60)
    print("训练配置 (RGBD 仅分类 - 消融实验)")
    print("=" * 60)
    print(f"实验名称: {config.exp_name}")
    print(f"任务: 仅分类 (无分割头)")
    print(f"输入: RGBD (5 通道)")
    print(f"模型: TransUNet-ConvNeXt-RGBD-ClsOnly-{config.model.variant}")
