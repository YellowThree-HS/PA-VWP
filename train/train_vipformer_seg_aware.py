"""
VIPFormer Seg-Aware 训练脚本 - RGB 单流版本

用法:
    python train_vipformer_seg_aware.py --config base
    python train_vipformer_seg_aware.py --config debug
    python train_vipformer_seg_aware.py --config base --epochs 200 --lr 1e-4

特点:
- 使用 VIPFormerSegAware 模型 (两阶段 Transformer)
...
from train.models.vipformer_seg_aware import (
    VIPFormerSegAware,
    VIPFormerSegAwareConfig,
    create_vipformer_seg_aware,
    vipformer_seg_aware_base,
    vipformer_seg_aware_small,
    vipformer_seg_aware_tiny,
)
...
def create_model_vipformer(config: Config, device: torch.device) -> nn.Module:
    ...
    if model_config.variant == 'tiny':
        ...
        model = vipformer_seg_aware_small(
            pretrained=model_config.pretrained,
            img_height=config.data.img_height,
            img_width=config.data.img_width,
            dropout=model_config.dropout,
        ...
        model = vipformer_seg_aware_base(
            pretrained=model_config.pretrained,
            img_height=config.data.img_height,
            img_width=config.data.img_width,
            dropout=model_config.dropout,
        ...
        # 自定义配置
        model = create_vipformer_seg_aware(
            pretrained=model_config.pretrained,
            img_height=config.data.img_height,
            img_width=config.data.img_width,
    ...
    print(f"模型: VIPFormer-SegAware-{model_config.variant}")
...
def get_config_vipformer(preset: str = 'default') -> Config:
    """获取 VIPFormer Seg-Aware 配置"""
    if preset == 'debug':
        return Config(
            exp_name='vipformer_seg_aware_debug',
            data=DataConfig(
        ...
        return Config(
            exp_name='vipformer_seg_aware_default',
            data=DataConfig(
                data_root='/DATA/disk0/hs_25/pa',
                train_dirs=['all_dataset/train'],
                val_dirs=['all_dataset/val'],
            ...
            model=ModelConfig(variant='tiny'),
            train=TrainConfig(epochs=150, lr=1e-4),
            loss=LossConfig(),
        )
...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='VIPFormer Seg-Aware 训练')
    parser.add_argument('--config', type=str, default='default',
                        choices=['default', 'debug'], help='配置预设')
...
def main():
    ...
    print(f"模型: VIPFormer-SegAware-{config.model.variant}")
