"""
VIPFormer RGBD Seg-Aware 训练脚本 - RGBD 双流版本

用法:
    python train_vipformer_rgbd_seg_aware.py --config base
    python train_vipformer_rgbd_seg_aware.py --config debug
    python train_vipformer_rgbd_seg_aware.py --config base --epochs 200 --lr 1e-4

特点:
- 使用 VIPFormerRGBDSegAware 模型 (两阶段 Transformer + 双流)
- RGB分支 + Depth分支，特征级融合
...
from train.models.vipformer_rgbd_seg_aware import (
    VIPFormerRGBDSegAware,
    VIPFormerRGBDSegAwareConfig,
    create_vipformer_rgbd_seg_aware,
    vipformer_rgbd_seg_aware_base,
    vipformer_rgbd_seg_aware_small,
    vipformer_rgbd_seg_aware_tiny,
)
...
def create_model_vipformer_rgbd(config: Config, device: torch.device) -> nn.Module:
    """创建 VIPFormer RGBD Seg-Aware 模型"""
    ...
    if model_config.variant == 'tiny':
        ...
        model = vipformer_rgbd_seg_aware_small(
            pretrained=model_config.pretrained,
            img_height=config.data.img_height,
            img_width=config.data.img_width,
            dropout=model_config.dropout,
        ...
        model = vipformer_rgbd_seg_aware_base(
            pretrained=model_config.pretrained,
            img_height=config.data.img_height,
            img_width=config.data.img_width,
            dropout=model_config.dropout,
        ...
        # 自定义配置
        model = create_vipformer_rgbd_seg_aware(
            pretrained=model_config.pretrained,
            img_height=config.data.img_height,
            img_width=config.data.img_width,
    ...
    print(f"模型: VIPFormer-RGBD-SegAware-{model_config.variant}")
...
def get_config_vipformer_rgbd(preset: str = 'default') -> Config:
    """获取 VIPFormer RGBD Seg-Aware 配置"""
    if preset == 'debug':
        return Config(
            exp_name='vipformer_rgbd_seg_aware_debug',
            data=DataConfig(
        ...
        return Config(
            exp_name='vipformer_rgbd_seg_aware_default',
            data=DataConfig(
                data_root='/DATA/disk0/hs_25/pa',
                train_dirs=['all_dataset/train'],
                val_dirs=['all_dataset/val'],
...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='VIPFormer RGBD Seg-Aware 训练')
...
def main():
    ...
    print(f"模型: VIPFormer-RGBD-SegAware-{config.model.variant}")
