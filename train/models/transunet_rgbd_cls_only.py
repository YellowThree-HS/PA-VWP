"""
TransUNet ResNet-50 RGBD 仅分类模型
用于多任务学习消融实验 - 移除分割头 (带深度图)
...
@dataclass
class TransUNetRGBDClsOnlyConfig(TransUNetRGBDConfig):
    """TransUNet ResNet-50 RGBD 仅分类配置"""
    pass


class TransUNetRGBDClsOnly(nn.Module):
    """
    TransUNet with ResNet-50 Encoder - RGBD 仅分类版本
    ...
    def __init__(self, config: Optional[TransUNetRGBDClsOnlyConfig] = None):
        super().__init__()
        
        if config is None:
            config = TransUNetRGBDClsOnlyConfig()
        self.config = config
...
def create_transunet_rgbd_cls_only(
    ...
    **kwargs
    ...
    return TransUNetRGBDClsOnly(config)


def transunet_rgbd_cls_only_base(pretrained: bool = True, **kwargs) -> TransUNetRGBDClsOnly:
    return create_transunet_rgbd_cls_only(
        pretrained=pretrained,
...
def transunet_rgbd_cls_only_small(pretrained: bool = True, **kwargs) -> TransUNetRGBDClsOnly:
    return create_transunet_rgbd_cls_only(
        pretrained=pretrained,
...
def transunet_rgbd_cls_only_tiny(pretrained: bool = True, **kwargs) -> TransUNetRGBDClsOnly:
    return create_transunet_rgbd_cls_only(
        pretrained=pretrained,
...
if __name__ == "__main__":
    model = transunet_rgbd_cls_only_base()
