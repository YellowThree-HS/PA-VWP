"""
TransUNet ResNet-50 RGBD 仅分割模型
用于多任务学习消融实验 - 移除分类头 (带深度图)
...
@dataclass
class TransUNetRGBDSegOnlyConfig(TransUNetRGBDConfig):
    """TransUNet ResNet-50 RGBD 仅分割配置"""
    pass


class TransUNetRGBDSegOnly(nn.Module):
    """
    TransUNet with ResNet-50 Encoder - RGBD 仅分割版本
    ...
    def __init__(self, config: Optional[TransUNetRGBDSegOnlyConfig] = None):
        super().__init__()
        
        if config is None:
            config = TransUNetRGBDSegOnlyConfig()
        self.config = config
...
def create_transunet_rgbd_seg_only(
    ...
    **kwargs
    ...
    return TransUNetRGBDSegOnly(config)


def transunet_rgbd_seg_only_base(pretrained: bool = True, **kwargs) -> TransUNetRGBDSegOnly:
    return create_transunet_rgbd_seg_only(
        pretrained=pretrained,
...
def transunet_rgbd_seg_only_small(pretrained: bool = True, **kwargs) -> TransUNetRGBDSegOnly:
    return create_transunet_rgbd_seg_only(
        pretrained=pretrained,
...
def transunet_rgbd_seg_only_tiny(pretrained: bool = True, **kwargs) -> TransUNetRGBDSegOnly:
    return create_transunet_rgbd_seg_only(
        pretrained=pretrained,
...
if __name__ == "__main__":
    model = transunet_rgbd_seg_only_base()
