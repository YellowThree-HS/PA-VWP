"""
TransUNet ConvNeXt RGBD 仅分割训练脚本
用于多任务学习消融实验 - 仅分割任务 (带深度图)

用法:
    python train_convnext_rgbd_seg_only.py --config base
    python train_convnext_rgbd_seg_only.py --config debug
...
from train.models.transunet_convnext_rgbd_seg_only import (
    TransUNetConvNeXtRGBDSegOnly, 
    TransUNetConvNeXtRGBDSegOnlyConfig,
    transunet_convnext_rgbd_seg_only_base, 
    transunet_convnext_rgbd_seg_only_small, 
    transunet_convnext_rgbd_seg_only_tiny
)
...
def create_model_rgbd_seg_only(config: Config, device: torch.device) -> nn.Module:
    """创建 RGBD 仅分割模型"""
    model_config = config.model
    
    if model_config.variant == 'tiny':
        model = transunet_convnext_rgbd_seg_only_tiny(
        ...
        model = transunet_convnext_rgbd_seg_only_small(
            pretrained=model_config.pretrained,
            img_height=config.data.img_height,
            img_width=config.data.img_width,
            dropout=model_config.dropout,
        ...
        model = transunet_convnext_rgbd_seg_only_base(
            pretrained=model_config.pretrained,
            img_height=config.data.img_height,
            img_width=config.data.img_width,
            dropout=model_config.dropout,
    ...
    print(f"模型: TransUNet-ConvNeXt-RGBD-SegOnly-{model_config.variant}")
    print(f"任务: 仅分割 (无分类头)")
...
def train(config: Config):
    ...
    model = create_model_rgbd_seg_only(config, device)
...
def get_config_rgbd_seg_only(preset: str = 'default') -> Config:
    """获取 RGBD 仅分割预设配置"""
    ...
    if preset == 'debug':
        return Config(
            ...
            exp_name='debug_convnext_rgbd_seg_only',
        )
    elif preset == 'base':
        return Config(
            ...
            exp_name='transunet_convnext_rgbd_seg_only_base',
        )
    else:  # default
        return Config(
            data=DataConfig(
            ...
            exp_name='transunet_convnext_rgbd_seg_only',
        )


def parse_args():
    parser = argparse.ArgumentParser(description='TransUNet ConvNeXt RGBD 仅分割训练')
...
def main():
    args = parse_args()
    config = get_config_rgbd_seg_only(args.config)
    ...
    print("\n" + "=" * 60)
    print("训练配置 (RGBD 仅分割 - 消融实验)")
    print("=" * 60)
    print(f"实验名称: {config.exp_name}")
    print(f"任务: 仅分割 (无分类头)")
    print(f"输入: RGBD (5 通道)")
    print(f"模型: TransUNet-ConvNeXt-RGBD-SegOnly-{config.model.variant}")
