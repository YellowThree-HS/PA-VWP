"""
TransUNet ConvNeXt RGBD 训练脚本 - 带深度图输入
使用 ConvNeXt 作为 CNN Encoder

用法:
    python train_convnext_with_depth.py --config base
    python train_convnext_with_depth.py --config debug
    python train_convnext_with_depth.py --config base --epochs 200 --lr 1e-4

与 ResNet 版本 (train_with_depth.py) 的区别:
- 使用 ConvNeXt 替代 ResNet-50 作为 backbone
- 使用 TransUNetConvNeXtRGBD 模型
...
from train.models.transunet_convnext_rgbd import (
    TransUNetConvNeXtRGBD, 
    TransUNetConvNeXtRGBDConfig,
    transunet_convnext_rgbd_base, 
    transunet_convnext_rgbd_small, 
    transunet_convnext_rgbd_tiny
)
...
def create_model_convnext_rgbd(config: Config, device: torch.device) -> nn.Module:
    """创建 ConvNeXt RGBD 模型"""
    model_config = config.model
    
    if model_config.variant == 'tiny':
        model = transunet_convnext_rgbd_tiny(
        ...
        model = transunet_convnext_rgbd_small(
            pretrained=model_config.pretrained,
            img_height=config.data.img_height,
            img_width=config.data.img_width,
            dropout=model_config.dropout,
        ...
        model = transunet_convnext_rgbd_base(
            pretrained=model_config.pretrained,
            img_height=config.data.img_height,
            img_width=config.data.img_width,
            dropout=model_config.dropout,
        ...
        # 自定义配置
        transunet_config = TransUNetConvNeXtRGBDConfig(
            img_height=config.data.img_height,
            img_width=config.data.img_width,
            pretrained=model_config.pretrained,
        ...
        model = TransUNetConvNeXtRGBD(transunet_config)
    ...
    print(f"模型: TransUNet-ConvNeXt-RGBD-{model_config.variant}")
...
def train(config: Config):
    ...
    # 创建模型 (ConvNeXt RGBD 版本)
    print("\n" + "=" * 60)
    print("创建模型 (ConvNeXt RGBD Backbone)...")
    print("=" * 60)
    
    model = create_model_convnext_rgbd(config, device)
...
def get_config_convnext_rgbd(preset: str = 'default') -> Config:
    """获取 ConvNeXt RGBD 预设配置"""
    ...
    if preset == 'default':
        return Config(
            ...
            exp_name='transunet_convnext_rgbd',
        )
        ...
        return Config(
            ...
            exp_name='transunet_convnext_rgbd_small',
        )
        ...
        return Config(
            ...
            exp_name='transunet_convnext_rgbd_base',
        )
        ...
        return Config(
            ...
            exp_name='transunet_convnext_rgbd_full',
        )
...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='TransUNet ConvNeXt RGBD 训练脚本 (带深度图)')
...
def main():
    ...
    # 加载 ConvNeXt RGBD 专用配置
    config = get_config_convnext_rgbd(args.config)
    ...
    # 打印配置
    print("\n" + "=" * 60)
    print("训练配置 (ConvNeXt RGBD - 带深度图)")
    print("=" * 60)
    print(f"实验名称: {config.exp_name}")
    print(f"Backbone: ConvNeXt-Base (现代化卷积网络)")
    print(f"输入类型: RGBD (5 通道: RGB + Depth + Target Mask)")
    ...
    print(f"模型: TransUNet-ConvNeXt-RGBD-{config.model.variant}")
