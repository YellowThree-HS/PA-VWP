"""
TransUNet ResNet-50 RGBD 仅分割训练脚本
用于多任务学习消融实验 - 仅分割任务 (带深度图)

用法:
    python train_rgbd_seg_only.py --config base
"""
...
from train.models.transunet_rgbd_seg_only import (
    TransUNetRGBDSegOnly,
    transunet_rgbd_seg_only_base,
    transunet_rgbd_seg_only_small,
    transunet_rgbd_seg_only_tiny,
)
...
class SegmentationOnlyLoss(nn.Module):
    ...
    def __init__(self, dice_smooth: float = 1.0, pos_weight: float = 10.0):
        super().__init__()
        # 正样本权重：因为 affected 区域通常很小，需要更大的权重
    ...
    def forward(self, seg_logits, seg_targets, stability_labels=None):
        ...
        # 如果提供了稳定性标签，只对不稳定样本计算 loss
        if stability_labels is not None:
            # 找出不稳定样本（label=0 表示不稳定，有受影响区域）
...
def create_model(config: Config, device: torch.device) -> nn.Module:
    ...
    if model_config.variant == 'tiny':
        model = transunet_rgbd_seg_only_tiny(pretrained=model_config.pretrained,
        ...
        model = transunet_rgbd_seg_only_small(pretrained=model_config.pretrained,
            img_height=config.data.img_height, img_width=config.data.img_width, dropout=model_config.dropout)
    else:
        model = transunet_rgbd_seg_only_base(pretrained=model_config.pretrained,
            img_height=config.data.img_height, img_width=config.data.img_width, dropout=model_config.dropout)
        
    model = model.to(device)
    print(f"模型: TransUNet-ResNet50-RGBD-SegOnly-{model_config.variant}")
    print(f"任务: 仅分割 (无分类头)")
...
def train_one_epoch(model, train_loader, criterion, optimizer, device, config, scaler=None, epoch=0):
    ...
    for batch in pbar:
        ...
        stability_labels = batch['stability_label'].to(device)  # 0=不稳定, 1=稳定
        ...
        if config.train.use_amp and scaler:
            ...
            # 在 autocast 外部计算 loss，强制使用 FP32
...
def get_config(preset: str = 'default') -> Config:
    ...
    if preset == 'debug':
        return Config(
            ...
            exp_name='debug_resnet_rgbd_seg_only')
    elif preset == 'base':
        return Config(
            ...
            train=TrainConfig(
                epochs=150, 
                lr=1e-5,           # 降低初始学习率，避免 NaN
                grad_clip=0.5, 
                warmup_epochs=3,   # 减少 warmup 步数
                save_dir=CHECKPOINTS_DIR
            ),
            exp_name='transunet_rgbd_seg_only_base')
    else:
        return Config(
            ...
            exp_name='transunet_rgbd_seg_only')
...
def main():
    ...
    print(f"\n{'='*60}\n消融实验: ResNet-50 RGBD 仅分割\n{'='*60}")
    print(f"模型: TransUNet-ResNet50-RGBD-SegOnly-{config.model.variant}")
    train(config)
