"""
VIPFormer Seg-Guided 训练脚本 - RGB 单流版本

用法:
    python train_vipformer_seg_guided.py --config base
    python train_vipformer_seg_guided.py --config debug
    python train_vipformer_seg_guided.py --config base --epochs 200 --lr 1e-4

特点:
- 使用 VIPFormerSegGuided 模型 (分割引导分类)
- SEG Query Tokens 在 Transformer 中学习分割语义
- Cross-Attention Fusion: CLS token 聚合分割信息
- Uncertainty-Aware Weighting: 根据分割置信度动态调整
"""
...
from train.models.vipformer_seg_guided import (
    VIPFormerSegGuided,
...
def create_model_vipformer(config: Config, device: torch.device) -> nn.Module:
    """创建 VIPFormer Seg-Guided 模型"""
    model_config = config.model

    # 获取 VIPFormer 特定配置
    fusion_type = getattr(model_config, 'fusion_type', 'cross_attention')
    num_seg_queries = getattr(model_config, 'num_seg_queries', 16)

    if model_config.variant == 'tiny':
        model = vipformer_seg_guided_tiny(
        ...
        model = vipformer_seg_guided_small(
            pretrained=model_config.pretrained,
            img_height=config.data.img_height,
            img_width=config.data.img_width,
            dropout=model_config.dropout,
        ...
        model = vipformer_seg_guided_base(
            pretrained=model_config.pretrained,
            img_height=config.data.img_height,
            img_width=config.data.img_width,
            dropout=model_config.dropout,
        ...
        # 自定义配置
        vipformer_config = VIPFormerSegGuidedConfig(
            img_height=config.data.img_height,
            img_width=config.data.img_width,
            pretrained=model_config.pretrained,
        ...
        model = VIPFormerSegGuided(vipformer_config)
    ...
    print(f"模型: VIPFormer-SegGuided-{model_config.variant}")
...
def train(config: Config):
    ...
    # 创建模型
    model = create_model_vipformer(config, device)
...
def get_config_vipformer(preset: str = 'default') -> Config:
    """获取 VIPFormer 配置"""
    if preset == 'debug':
        return Config(
            exp_name='vipformer_seg_guided_debug',
            data=DataConfig(
        ...
        return Config(
            exp_name='vipformer_seg_guided_small',
            data=DataConfig(
                data_root='/DATA/disk0/hs_25/pa',
                train_dirs=['all_dataset/train'],
                val_dirs=['all_dataset/val'],
        ...
        return Config(
            exp_name='vipformer_seg_guided_base',
            data=DataConfig(
                data_root='/DATA/disk0/hs_25/pa',
                train_dirs=['all_dataset/train'],
                val_dirs=['all_dataset/val'],
        ...
        return Config(
            exp_name='vipformer_seg_guided_default',
            data=DataConfig(
                data_root='/DATA/disk0/hs_25/pa',
                train_dirs=['all_dataset/train'],
                val_dirs=['all_dataset/val'],
...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='VIPFormer Seg-Guided 训练 (RGB)')

    parser.add_argument('--config', type=str, default='default',
                        choices=['default', 'debug', 'small', 'base'])
...
def main():
    ...
    # 加载配置
    config = get_config_vipformer(args.config)
    ...
    # 打印配置
    print("\n" + "=" * 60)
    print("VIPFormer Seg-Guided 训练配置 (RGB)")
    print("=" * 60)
    print(f"实验名称: {config.exp_name}")
    print(f"模型: VIPFormer-SegGuided-{config.model.variant}")
