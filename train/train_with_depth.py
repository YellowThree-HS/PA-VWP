"""
TransUNet RGBD 训练脚本 - 带深度图输入

用法:
    python train_with_depth.py --config base
    python train_with_depth.py --config debug
    python train_with_depth.py --config base --epochs 200 --lr 1e-4

与原版 train.py 的区别:
- 使用 5 通道输入 (RGB + Depth + Target Mask)
- 使用 BoxWorldDatasetWithDepth 数据集
- 使用 TransUNetRGBD 模型
"""
...
def create_model_rgbd(config: Config, device: torch.device) -> nn.Module:
    """创建 RGBD 模型"""
    model_config = config.model
    
    if model_config.variant == 'tiny':
        model = transunet_rgbd_tiny(
        ...
        # 自定义配置
        transunet_config = TransUNetRGBDConfig(
            img_height=config.data.img_height,
            img_width=config.data.img_width,
            pretrained=model_config.pretrained,
        ...
        model = TransUNetRGBD(transunet_config)
    ...
    print(f"模型: TransUNet-RGBD-{model_config.variant}")
...
def train(config: Config):
    ...
    model = create_model_rgbd(config, device)
...
def get_config_rgbd(preset: str = 'default') -> Config:
    """获取 RGBD 预设配置"""
    
    # 统一的保存目录：使用数据目录下的 checkpoints
...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='TransUNet RGBD 训练脚本 (带深度图)')
...
def main():
    ...
    # 加载 RGBD 专用配置
    config = get_config_rgbd(args.config)
    ...
    print(f"模型: TransUNet-RGBD-{config.model.variant}")
