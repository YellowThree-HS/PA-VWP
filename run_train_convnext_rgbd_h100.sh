...
# ==========================================
# 使用方法：
#   方式1 (后台，默认): bash run_train_convnext_rgbd_h100.sh
#   方式2 (前台):       bash run_train_convnext_rgbd_h100.sh --foreground 或 -f
#   方式3 (SLURM):      sbatch run_train_convnext_rgbd_h100.sh  # 需要添加 SLURM 头
#   方式4 (交互式):     srun -p student --gres=gpu:1 --cpus-per-task=32 --mem=64G --pty bash run_train_convnext_rgbd_h100.sh --foreground
# ==========================================
# 
# 与 ResNet 版本的区别：
...
#   - 通常比 ResNet-50 性能更强
#   - 实验名称为 transunet_convnext_rgbd_base_h100
# ==========================================
...
echo -e "${CYAN}=========================================="
echo "H100 训练脚本 - TransUNet ConvNeXt RGBD"
echo -e "==========================================${NC}"
echo ""
echo "ConvNeXt 特点:"
echo "  - 现代化卷积网络设计"
...
echo ""
echo -e "${MAGENTA}输入: 5 通道 (RGB + Depth + Target Mask)${NC}"
echo ""
...
# Batch Size 计算
# ConvNeXt RGBD: ConvNeXt 占用更多显存，加上多一个深度通道
if [ "$IS_H100" = true ]; then
    BATCH_SIZE=14  # ConvNeXt RGBD 需要更小的 batch size
else
    # 提取显存 GB 数（去除单位）
...
# 例如: RESUME_PATH="/DATA/disk0/hs_25/pa/checkpoints/transunet_convnext_rgbd_base_h100/checkpoint_epoch_7.pth"
RESUME_PATH=""
...
# ==========================================
echo -e "\n${CYAN}=========================================="
echo "[5/5] 开始训练 TransUNet-ConvNeXt-RGBD-Base"
echo -e "==========================================${NC}"
echo ""
echo "训练参数:"
echo "  --config base"
...
echo -e "  ${CYAN}模型保存路径: /DATA/disk0/hs_25/pa/checkpoints/transunet_convnext_rgbd_base_h100/${NC}"
...
LOG_FILE="logs/train_transunet_convnext_rgbd_base_h100_${TIMESTAMP}.log"
PID_FILE="logs/train_transunet_convnext_rgbd_base_h100_${TIMESTAMP}.pid"
...
# 构建训练命令 (使用 train_convnext_with_depth.py)
TRAIN_CMD="python train/train_convnext_with_depth.py \
    --config base \
    --model base \
    ...
    --epochs 150 \
    --lr 5e-5 \
    --exp_name transunet_convnext_rgbd_base_h100"
