#!/bin/bash
# ============================================================
# TransUNet RGBD Cls-Only Tiny 训练脚本 (H100)
# Tiny 模型: 4 层 Transformer, 256 hidden dim (轻量级)
# RGBD: RGB + Depth 融合 (5通道输入)
# Cls-Only: 仅分类头，无分割头
# ============================================================
# 用法：
#   bash run_train_rgbd_cls_only_tiny_h100.sh        # 后台运行
#   bash run_train_rgbd_cls_only_tiny_h100.sh -f     # 前台运行
...
echo "=============================================="
echo "TransUNet RGBD Cls-Only Tiny - H100"
echo "=============================================="
echo "开始时间: $(date)"
...
# ========== 超参数 ==========
MODEL_VARIANT="tiny"
BATCH_SIZE=40              # tiny + cls_only，显存占用较小
EPOCHS=150
LR=1e-4
NUM_WORKERS=16
# 注意：train_rgbd_cls_only.py 内部默认保存到 /DATA/disk0/hs_25/pa/checkpoints
EXP_NAME="transunet_rgbd_cls_only_tiny_h100"

# ========== 运行训练 ==========
echo "实验名: ${EXP_NAME}"
echo "模型: ${MODEL_VARIANT} (4层 Transformer, 256 hidden dim)"
echo "输入: RGBD 5通道融合"
echo "特点: 仅分类头 (无分割头)"
